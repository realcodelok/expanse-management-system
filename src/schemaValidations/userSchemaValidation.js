...
exports.transactionSchema = () => {
  const schema = Joi.object({
    userId: Joi.objectId().empty().required().messages({
      "string.base": `User Id should be a type of 'objectId'.`,
      "string.empty": `User Id should not be empty.`,
      "string.pattern.name": `User Id is not valid.`,
      "any.required": `User Id is required.`,
    }),

    amount: Joi.number().min(1).required().messages({
      "number.base": `Amount should be a type of 'number'.`,
      "number.min": `Amount must be minimum {#limit}`,
      "any.required": `Please provide the amount.`,
    }),
    // type: Joi.string().valid('CREDIT', 'DEBIT', 'SHOPPING')
    //   .messages({
    //     "any.only": `Payment type must be one of [CREDIT, DEBIT, SHOPPING]`,
    //   }),
    ...
    admin: Joi.objectId().empty().required().messages({
      "string.base": `Admin Id should be a type of 'objectId'.`,
      "string.empty": `Admin Id should not be empty.`,
      "string.pattern.name": `Please provide a valid Admin Id.`,
      "any.required": `Please provide the Admin Id.`,
    }),
  });
  return schema;
};
exports.duesPaymentSchema = () => {
  const schema = Joi.object({
    duesId: Joi.objectId().empty().required().messages({
      "string.base": `Dues Id should be a type of 'objectId'.`,
      "string.empty": `Dues Id should not be empty.`,
      "string.pattern.name": `Dues Id is not valid.`,
      "any.required": `Dues Id is required.`,
    }),
    userId: Joi.objectId().empty().required().messages({
      "string.base": `User Id should be a type of 'objectId'.`,
      "string.empty": `User Id should not be empty.`,
