...
exports.validateSchema = async (req, res, next) => {
  try {
    ...
    if (e.details) {
      ...
      e.message = details.message.replace(/["]/g, "");
    }
    next(e);
  }
};
...
exports.creditRequest = (req, res, next) => {
  req.body.type = transactionTypes.credit;
  next();
};

exports.debitRequest = (req, res, next) => {
  req.body.type = transactionTypes.debit;
  next();
};

exports.shoppingRequest = (req, res, next) => {
  req.body.type = transactionTypes.shopping;
  next();
};

exports.validateNewTransactionSchema = async (req, res, next) => {
  try {
...
exports.validateDuesPaymentRequest = async (req, res, next) => {
  try {
    await duesPaymentSchema().validateAsync(req.body);
    return next();
  } catch (e) {
    if (e.details) {
      const details = e.details[0];
